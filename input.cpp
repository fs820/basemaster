	...
	if ((nControllerNum+XcontrollerNum)<=CONTROLLER_MAX)
	{
		...
		if (XInputGetState(XcontrollerNum, &state) == ERROR_SUCCESS)
		{
			...
			{//インプットデバイスの作成
				...
				strncpy_s(g_ConName[nControllerNum], sizeof(g_ConName[nControllerNum]), pdidInstance->tszProductName, _TRUNCATE);
				XcontrollerNum++;
			}
		}
		...
		{//インプットデバイスの作成
			...
			strncpy_s(g_ConName[nControllerNum + XNum()], sizeof(g_ConName[nControllerNum]), pdidInstance->tszProductName, _TRUNCATE);
			nControllerNum++;
		}
	}
...
// 入力を検出する関数
//*********************************
bool IsKeyboardUsed(void)
{
	for (int nCntKey = 0; nCntKey < NUM_KEY_MAX; nCntKey++)
	{
		if (g_akeyState[nCntKey] != 0)
		{
			return true; // どれかのキーに変化があった
		}
	}
	return false; // 変化なし
}
...
bool IsMouseUsed(void)
{
	...
	if (*GetMouseMove() != 0.0f && *(GetMouseMove() + 1) != 0.0f)
	{
		return true;
	}
	return false;
...
// 入力を検出する関数
//*********************************
bool IsJoyPadUsed(void)
{
	for (int nCnt = 0; nCnt < CONTROLLER_MAX; nCnt++)
	{
		// ボタンの変化チェック
		for (int nCntKey = 0; nCntKey < JOYKEY_MAX; nCntKey++)
		{
			if (GetJoykeyPress((JOYKEY)nCntKey, (CONTROLLER)nCnt))
			{
				return true;
			}
		}
		...
		{
			for (int nCNtDire = 0; nCNtDire < DIRESTICK_MAX; nCNtDire++)
			{
				if (JoyStickPress((DIRESTICK)nCNtDire, (STICK)nCNtStick, (CONTROLLER)nCnt))
				{
					return true;
				}
			}
